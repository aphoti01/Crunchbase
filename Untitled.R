...
edges <- data.frame(id1=es$head,id2=es$tail, time=es$onset)

#generate the full graph
g <- graph.data.frame(edges,directed=F)

test<- rbind(data.frame(id=edges$id1,onset=edges$time)
             ,data.frame(id=edges$id2,onset=edges$time))
onset<- test %>%
  group_by(id)%>%
  summarise(onset=min(onset))

ids<- data.frame(id=vertex_attr(g)$name)
ids$onset<- with(onset, onset[match(ids$id, id)])
vertex_attr(g)$onset<- ids$onset

#generate a cool palette for the graph (darker colors = older nodes)
#YlOrBr.pal <- colorRampPalette(brewer.pal(100,"YlOrRd"))
#colors for the nodes are chosen from the very beginning
pal <- colorNumeric(c("gold","red","darkorchid4"), 0:max(vertex_attr(g)$onset))
V(g)$color <- pal(vertex_attr(g)$onset)
